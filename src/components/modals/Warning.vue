    ...
    <div class="flex h-screen justify-center items-center">
      <div class="align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
        ...
        <div v-if="leftButon || rightButton" class="bg-gray px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-3">
          <custom-button :disabled="isLoading" v-if="rightButton" @click="rightButtonClicked()" class="w-full inline-flex justify-center sm:w-auto" :text="rightButton" type="button" />
          <custom-button :disabled="isLoading" v-if="leftButton" @click="leftButtonClicked()" class="w-full inline-flex justify-center sm:w-auto" :text="leftButton" type="button" />
        </div>
      </div>
    </div>
  </div>
