...
function useValidationState({ name, initValue, form, type, checkedValue, standalone }) {
  ...
  function resetValidationState(state) {
    ...
    const newValue = state && 'value' in state ? state.value : getFromPath(unref(formInitialValues), fieldPath, unref(initValue))
    if (form) {
...
export { script as CustomInput, script$2 as HelloWorld, script$1 as MyName }
